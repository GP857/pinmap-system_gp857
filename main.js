// A declaração `import { MarkerClusterer } from ...` foi removida.
// A biblioteca é carregada via `<script>` tag no HTML e está disponível globalmente.
// Lembre-se: `MarkerClusterer` agora é uma variável global, acessível diretamente.

// A função initMap é atribuída ao objeto `window` para que o callback da API do Google Maps possa encontrá-la.
window.initMap = async () => {
    // Código existente da função initMap
    // ... (o restante do seu código JavaScript, incluindo a criação do mapa e dos marcadores)
